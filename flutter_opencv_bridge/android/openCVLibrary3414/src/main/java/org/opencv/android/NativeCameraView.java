...
public class NativeCameraView extends CameraBridgeViewBase {
    ...
    private boolean initializeCamera(int width, int height) {
        synchronized (this) {
            ...
            boolean opened = mCamera.open(localCameraIndex); // próbálja meg az alap backendet
            if (!opened) {
                // fallback – néha az üres konstruktor + open működik, de ha mégsem, dobjunk hibát
                throw new RuntimeException("Failed to open camera index " + localCameraIndex);
            }
            if (mCamera == null)
                return false;
